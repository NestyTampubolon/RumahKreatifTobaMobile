...
class _KeranjangPageState extends State<KeranjangPage> {
  ...
  void initState() {
    ...
    map = Map.fromIterable(
      Get.find<CartController>().merchantKeranjangList.where((item) => item.namaMerchant != null),
                               ...
                               Row(
                                 children: [
                                   Checkbox(
                                       ...
                                       value: cartController.getMerchantCheckedStatus(merchantName) &&
                                           merchantItems.every((item) =>
                                               cartController.getCartCheckedStatus(item.productId)),
                                       onChanged: (bool? value) {
                                         // Set the checked status for the merchant
                                         cartController.setMerchantCheckedStatus(merchantName, value);

                                         // Update the checked status for all items in the merchant's list
                                         for (var item in merchantItems) {
                                           cartController.setCartCheckedStatus(item.productId, value);
                                         }
                                         setState(() {});
                                       }
                                   ),
                                   ...
                                   itemBuilder: (_, index) {
                                     ...
                                     return Row(
                                       children: [
                                         Checkbox(
                                             activeColor: AppColors.redColor,
                                             value: cartController.getCartCheckedStatus(item.productId),
                                             onChanged: (bool? value) {
                                               cartController.setCartCheckedStatus(item.productId, value);
                                               setState(() {
                                                 ...
                                                 if (value == true && !cartController.checkedCartIds.contains(item.productId)) {
                                                   cartController.checkedCartIds.add(item.cartId);
                                                 }
                                                 // If the checkbox is unchecked, remove the productId from the list
                                                 else if (value == false) {
                                                   cartController.checkedCartIds.remove(item.cartId);
                                                 }
                                               });
                                             }
                                         ),
